...
figure('Color',[1 1 1],'Units','inches','Position',[.5 .5 6 4]);
...
axis([min([Z]), max([Z]), min([W]), max([W])])

% If portfolio is not a double sort, just plot the portfolio number
%     for v=1:length(X)
%         text(X(v),Y(v),num2str(v),'Color','blue','FontSize',8,'FontWeight','bold')
%     end
% Same for OLS betas
for v=1:length(Z)
    text(Z(v),W(v),num2str(v),'Color','red','FontSize',10,'FontWeight','bold')
end
...
%             'EdgeColor','none')
annotation('textbox',[.9 .5 .1 .2],'String',{'\color{red} OLS','\color{black} - CAPM Implied'},...
    'EdgeColor','none')
% Add a CAPM line
...
export_fig(strcat('Port_Size_OLS','.pdf'));
close(gcf);
%%%
figure('Color',[1 1 1],'Units','inches','Position',[.5 .5 6 4]);
...
% If portfolio is not a double sort, just plot the portfolio number
for v=1:length(X)
    text(X(v),Y(v),num2str(v),'Color','blue','FontSize',10,'FontWeight','bold')
end
% Same for OLS betas
for v=1:length(Z)
    text(Z(v),W(v),num2str(v),'Color','red','FontSize',10,'FontWeight','bold')
...
export_fig(strcat('Port_Size_OLS_IV_NoOrth','.pdf'));
close(gcf);
...
Line_2 = [OLS_Line_Slopes(1,1), OLS_Line_Intercepts(1,1)];
% Axis is tight, can be changed
axis([min([Z;X]), max([X;Z]), min([W;Y]), max([W;Y])])
...
export_fig(strcat('Port_Size_OLS_IV','.pdf'));
close(gcf);
%%%%
...
Line_2 = [OLS_Line_Slopes(1,1), OLS_Line_Intercepts(1,1)];
% Axis is tight, can be changed
axis([min([Z;X]), max([Z;X]), min([W;Y]), max([W;Y])])
...
%             '\color{black} - CAPM Implied'},...
%             'EdgeColor','none')
annotation('textbox',[.9 .5 .1 .2],'String',{'\color{red} OLS', ...
    '\color{blue} - IV','\color{black} - CAPM Implied'},...
    'EdgeColor','none')
% Add a CAPM line
hline = refline([1200 .* mean(Mkt_Mean), 0]);
% Color black for the line
...
export_fig(strcat('Port_Beta_OLS','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Beta_Orth_NoOrth','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Beta_Orth','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Beta_NoOrth','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Beta_OLS_IV','.pdf'));
close(gcf);
%%%%
figure('Color',[1 1 1],'Units','inches','Position',[.5 .5 6 4]);
...
for v=1:length(X)
    ...
    text(X(v),Y(v),num2str(Port_N(v)),'Color','blue','FontSize',10,'FontWeight','bold')
end
% Also do this for OLS betas
for v=1:length(X)
    text(Z(v),W(v),num2str(Port_N(v)),'Color','red','FontSize',10,'FontWeight','bold')
end
...
export_fig(strcat('Port_Size_Beta_OLS_IV','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Size_BM_OLS_IV','.pdf'));
close(gcf);
...
export_fig(strcat('Port_Size_BM_OLS_IV_NoOrth','.pdf'));
close(gcf);

%% Make the five lines beta bias
load Beta_Table.mat

C = linspace(0.3,0.7,5);

X = OLS_Betas_SBT(:,1) - IV_Betas_SBT(:,1);
X_New = transpose(reshape(X,5,5));
...
ind1 = 1;
for b =1:5
    p(b) = plot(1 : 5, X_New(:, b), 'LineWidth',3,'Color',[C(b) C(b) C(b)]);
    hold on
    if b == 1 || b == 5
        
        plot(1 : 5, [X_New(:, b) + 1.96 .* Beta_Table(ind, 1 : end - 1)' ...
            X_New(:, b) - 1.96 .* Beta_Table(ind, 1 : end - 1)'],'--','Color',[C(b) C(b) C(b)])
           hold on
        ind = ind + 8;
    ...
    str{b} = ['Beta Quintile' ' ' num2str(b)];
    
    
end
plot(1 : 5, zeros(1, 5), '--','Color','k')
...
export_fig(strcat('Beta_Bias_Size_Bin','.pdf'));
close(gcf);
...
%
% figure('Color',[1 1 1],'Units','inches','Position',[1 1 9 3]);
...
% ylabel('Mean Excess Return')
% orient(gcf,'landscape');
% export_fig(strcat('Big_Fig_1','.pdf'));
% close(gcf);
%
% Fig =figure('Color',[1 1 1],'Units','inches','Position',[1 1 11 8.5]);
% %%%%%%%%%%%%%%%%%%%%%%%%%
...
% close(gcf);
%
% Fig =figure('Color',[1 1 1],'Units','inches','Position',[1 1 11 7]);
%
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
